...
# this is the simulated data, so that we know the true model, which is 
# y = 5 - 2x + x^2 + 2x^3 + \epsilon, \quad 
# \epsilon \sim Normal(\mu=0, \sigma^2 = 1).
...
# the figures require ggplot2 library and all packages it depends on.
library(ggplot2)

# generate the x predictor
x <- runif(100, -2, 2)
# generate the y response
y <- 2*x^3 + x^2 - 2*x + 5 + rnorm(100, 0, 1)
plot(x, y)
...
for(i in 1:max.poly){
    ...
    y_predict <- predict(lm(y~poly(x, i)), newdata=data.frame(x=x.new))
    new.dat[new.dat$degree==i,3] <- y_predict
    }

i = 1

ols_poly <- function(x, y, k){
